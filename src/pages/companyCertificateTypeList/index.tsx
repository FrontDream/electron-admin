...
import {
  ...
  addCompanyCertificateTypeApi,
  deleteCompanyCertificateTypeApi,
  updateCompanyCertificateTypeApi,
} from '@/services';
...
const CompanyCertificateTypeList: React.FC = () => {
  ...
  const onFinish = async (value: DepartmentData) => {
    ...
    try {
      ...
      if (isDdd) {
        res = await addCompanyCertificateTypeApi({ name });
      } else {
        res = await updateCompanyCertificateTypeApi({ name, id: currentRow?.id || 0 });
      }

      if (isSuccess(res, `${isDdd ? '新增' : '修改'}企业证书类型失败，请重试！`)) {
        message.success(`${isDdd ? '新增' : '修改'}企业证书类型成功`);
  ...
  const handleRemove = async (record: CertificateTypeItem) => {
    ...
    const del = async () => {
      ...
      try {
        const res = await deleteCompanyCertificateTypeApi(id);
    ...
    if (is_exists_cert) {
      warning({
        ...
        content: `该企业证书类型正在使用中，请先前往企业证书列表删除 ${rel_cert_list.join(
          ',',
        )} 等企业证书或修改这些企业证书的所属企业证书类型后重试!`,
      });
      return;
    }
    confirm({
  ...
  return (
    <PageContainer>
      <ProTable<CertificateTypeItem, TableListPagination>
        ...
        request={getCompanyCertificateTypeListApi}
        columns={columns}
...
export default CompanyCertificateTypeList;
